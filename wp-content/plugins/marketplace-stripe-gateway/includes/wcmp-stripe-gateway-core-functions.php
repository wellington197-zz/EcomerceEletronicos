...
if (!function_exists('marketplace_stripe_deprecated')) {

    function marketplace_stripe_deprecated() {
        ?>
        <div id="message" class="error">
            <p><?php printf(__('%sMarketplace Stripe Gateway plugin is inactive.%s Marketplace Stripe Gateway is now merged with WC Marketplace core from version 3.1.0.', 'saved-cards'), '<strong>', '</strong>'); ?></p>
        </div>
        <?php
    }
}
