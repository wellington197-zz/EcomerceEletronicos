...
(function( $ ) {
	...
	wp.customize( 'header_textcolor', function( value ) {
		value.bind( function( to ) {
			if ( 'blank' === to ) {
				...
				if ( ! to.length ) {
					$( '#sh_allo-custom-header-styles' ).remove();
				}
				$( '.site-title, .site-description' ).css({
					clip: 'auto',
					position: 'relative'
	...
	// Changing Header Top Part
	wp.customize( 'header_location_text', function( value ) {
		value.bind( function( to ) {
			$( '.header-top-location .location-text' ).text( to );
		});
	} );	

	wp.customize( 'header_phone_text', function( value ) {
		value.bind( function( to ) {
			$( '.header-top-phone .phone-text' ).text( to );
		});
	} );
} )( jQuery );
