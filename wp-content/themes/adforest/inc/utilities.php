...
function add_more_custom_layouts() {
  ...
  'cells' => '16_16_16_12',
  'mask' => '424',
  'title' => 'Custom 1/6 + 1/6 + 1/6 + 1/2 + Humayun',
	...
	function adforest_search_params( $index, $second = '', $third = '', $search_url = false)
	{
		...
		if( isset( $param ) && $index != 'cat_id' )
		{
			...
			foreach( $vars as $key => $val )
			{
				...
				if($second != "" ) { if ( $key == $second ) {continue;	} }
				if($third != "" ) { if ( $key == $third ) {continue;	} }
...
function adforest_video_icon(  $is_grid2 = false, $class = 'play-video' )
{
	global $adforest_theme;
	...
	{
		return '<a href="'.get_post_meta(get_the_ID(), '_adforest_ad_yvideo', true ).'" class="'.esc_attr($class).' '.esc_attr($fet_cls).'"><i class="fa fa-play-circle-o"></i></a>';	
	}	
}
}
...
if ( ! function_exists( 'adforest_social_profiles' ) ) {
	function adforest_social_profiles()
	{
		...
		{
			$social_netwroks 	=	 array( 'facebook' => __('Facebook','adforest'), 'twitter' => __('Twitter','adforest') ,'linkedin' => __('Linkedin','adforest'), 'google' => __('Google+','adforest'));
		}
		else
		{
...
{  
	...
	{
		$comments = ''; $get_rating_avrage = ''; $one_star = ''; $two_star = ''; $three_star = ''; $four_star = ''; $five_star = '';
		...
		if(count($comments) > 0 )
		{
			...
			$sum_of_rated = 0; $no_of_times_rated = 0; 
...
function adforest_getLatLong( $address = ''){
    ...
    if( $address ){
        ...
        $geocodeFromAddr = file_get_contents('https://maps.googleapis.com/maps/api/geocode/json?key='.$adforest_theme['gmap_api_key'].'&address='.$formattedAddr.'&sensor=false', false, stream_context_create($arrContextOptions)); 
...
function adforest_timer_html( $bid_end_date, $show_unit = true  )
{
	global $adforest_theme;
	...
	if( $bid_end_date == "")
		...
		if( $show_unit )
		{
			...
			$seconds	= '<div class="text">'.__('Seconds','adforest').'</div>';
				...
				$mt_rand	= mt_rand();
				$html = '
				<div class="clock" data-rand="'.esc_attr($mt_rand).'" data-date="'.$bid_end_date.'">
